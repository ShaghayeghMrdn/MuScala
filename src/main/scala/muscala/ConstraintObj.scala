...
//      var second: Operand = null;
//      def constructFromTree(expr: Tree): Unit = {
//        var tokens = expr match {
//          case Apply(Select((arg1 @ _), (opr @ _)), List((arg2 @ _))) => scala.Tuple3(operandConvert(arg1), opConvert(opr.toString()), operandConvert(arg2))
//          case Select(_) => scala.Tuple3(operandConvert(expr), new EqualTo(), new ConstVal("True"))
...
//        second = tokens._3
//      };
//      def operandConvert(node: Tree): Operand = node match {
//        case Literal(Constant((x @ _))) => ConstVal(x.toString())
//        case Ident((y @ _)) => Variable(y.toString())
//        case Select(Ident((w @ _)), (z @ _)) => Variable(w.toString().-('.').-(z.toString()))
//        case (x @ _) => {
//          println(showRaw(x));
//          throw new BadMatchException("Matching arg failed!")
//        }
//      };
...
//      };
//      def isNegationOf(that: Constraint): Boolean = return this.first.equals(that.first).&&(this.second.equals(that.second)).&&(this.op.getClass.==(that.op.negation.getClass));
//      override def toString(): String = {
//        if (second.toString().==("true"))
//          return first.toString().-(op.toString()).-("\"").-(second.toString()).-("\"")
//        else
//          ();
//        return first.toString().-(op.toString()).-(second.toString())
//      };
//      def <(that: Constraint): Boolean = return new StringOps(this.toString()).<(that.toString())
//    }
//  };
